...
public class ClientConnection {
	...
	public static ClientConnection createClientConnection() {
		...
		try {
			res.conn = new SSLServerConnection(res.clientConfig.hostAddress,
					res.clientConfig.portNumber,
					res.clientConfig.maxWaitTimeForHandshakeToComplete,
					res.clientConfig.keyStorePath, res.clientConfig.passPhrase
							.toCharArray());
	...
	public void init(String configFileName) throws FileNotFoundException,
		...
		elementConfig.loadFromFile(clientConfig.packetConfigPath + "/"
				+ clientConfig.packetElementsFileName);
		ElementConfig headerConfig = new ElementConfig();
		headerConfig.loadFromFile(clientConfig.packetConfigPath + "/"
				+ clientConfig.packetHeaderFileName);
		PacketConfig packetConfig = new PacketConfig();
		packetConfig.loadFromDirectory(clientConfig.packetConfigPath + "/"
				+ clientConfig.packetDirectory, elementConfig);
	...
	public int sendData(XMLWrap packetHeaderXmlSend, XMLWrap packetXmlSend) {
		...
		// 构造发送数据
		bufSend = parser.xmlToBytes(packetHeaderXmlSend, packetXmlSend,
		...
		if ("000000".equals(retCode) && recvData.length > headerByteLength) {
			...
			System.arraycopy(recvData, headerByteLength, packetData, 0,
					recvData.length - headerByteLength);
			//System.out.println("Received Packet:["+new String(packetData)+"]");

			trCode = headerXml.getInputValue("TRCODE");
			this.packetXml = parser.bytesToPacketXml(packetData, trCode,
