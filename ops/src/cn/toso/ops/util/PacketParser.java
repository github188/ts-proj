...
public class PacketParser {
    ...
    public byte[] xmlToBytes(XMLWrap packetHeaderXml, XMLWrap packetXml, int packetFlag) {
	/*
	...
	if (PACKET_FLAG_REQUEST == packetFlag) {
		...
		for (int item = 0; item < elements.size(); item++) {
		    ...
		    value = packetXml.getItemValue(rowName, row + 1, element.name);
	...
	for (int i = 0; i < elements.size(); i++) {
	    ...
	    addToXml(res, element.name, 1, element, headerBytes, from, to, PACKET_FLAG_REQUEST);
    ...
    public XMLWrap bytesToPacketXml(byte[] packetBytes, String trCode, int packetFlag) {
	XMLWrap res = new XMLWrap();
	...
	if (PACKET_FLAG_REQUEST == packetFlag) {
		...
		for (int item = 0; item < elements.size(); item++) {
		    ...
		    addToXml(res, element.name, 1, element, packetBytes, from, to, packetFlag);
		...
		for (int item = 0; item < elements.size(); item++) {
		    ...
		    addToXml(res, rowName, row + 1, element, packetBytes, from, to, packetFlag);
    ...
    private void addToXml(XMLWrap xml, String rowName, int row, PacketElement element, byte[] byteValue,
	    int from, int to, int packetFlag) {
    ...
    private void addToStream(ByteArrayOutputStream baos, PacketElement element, String value) {
	byte fillChar = element.fillCharacter;
	byte[] byteValue;
	int fillCharCount;
	try {
