...
public class WorkRunnable implements Runnable {
    ...
    public WorkRunnable(ExchangeInfo exchangeInfo, PacketParser parser, ControlPool controlPool) {
	...
	try {
		...
		if (exInfo.getRemainLength() == 0) {
		    // 若读完Header***************************************
		    // 解析Header
		    headerXml = parser.bytesToHeaderXml(exInfo.getHeaderData().toByteArray());
		    exInfo.setHeaderXml(headerXml);

		    // startPacket(ParseResult.length),
		    packetLength = Integer.parseInt(headerXml.getInputValue("PACKETLENGTH"));
	...
	if (logger.isInfoEnabled()) {
	    logger.info("Received Header:[" + new String(exInfo.getHeaderData().toByteArray()) + "]");
	    logger.info("Received Packet:[" + new String(exInfo.getPacketData().toByteArray()) + "]");
	}
	...
	// 解析Packet
	packetXml = parser.bytesToPacketXml(bPacketData, trCode, PacketParser.PACKET_FLAG_REQUEST);
	// 设置FUNC_ID
	...
	try {
	    ...
	    headerXml.setInputValue("AuthServerConnPara", 1, eapApiControl.authServerConnPara);

	    // 序号
	    if (headerXml.getInputRowCount("SEQ_NO") == 0) {
		headerXml.addInputRow("SEQ_NO");
	    }
	    headerXml.setInputValue("SEQ_NO", 1, SeqNoGennor.createGennor().next());
	...
	// BGN\t"+System.currentTimeMillis());
	dataSend = parser.xmlToBytes(headerXml, packetXml, PacketParser.PACKET_FLAG_RESPONSE);
	// System.out.println("TIME SEND:XML TO BYTES
	// END\t"+System.currentTimeMillis());
