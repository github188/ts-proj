...
public class BoDeviceTypeSubmit implements RootBo{
	public void doBusiness(Transaction transaction, XMLWrap requestXml, XMLWrap sessionXml,
	    ...
	    **********************************************************************/
		...
		sql1 = new StringBuffer("select * from DEVICE_TYPE where TYPE_NAME_EN ='" + enDeviceType.getTypeNameEn() + "'");
		sql2 = new StringBuffer("select * from DEVICE_TYPE where  TYPE_NAME_CN ='" + enDeviceType.getTypeNameCn() + "'");
		try {
		    conn = transaction.getConnById(null);
		    stm = conn.createStatement();
		    if (enDeviceType.getTypeId() == null || enDeviceType.getTypeId().length() == 0) {

		    	enDeviceType.setTypeId(SysIdCreator.GenNextId(transaction, null, IdCreatorDefine.ID_TYPE_DEVICE_TYPE_ID,
