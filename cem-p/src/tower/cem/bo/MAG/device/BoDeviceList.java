	...
	public void doBusiness(Transaction transaction, XMLWrap requestXml, 
		 ...
		 **********************************************************************/
         ...
         //查询表，将符合条件的保存到requestXml中返回。
			...
			dbDeviceInfo.setAllToXml(requestXml, devices);
			for(int i=0;i<devices.size();i++){
				...
				if(enFrontHostInfo != null ){
					row = dbFrontHostInfo.setToXml(requestXml, enFrontHostInfo);
					requestXml.setItemValue("DEVICE_INFO", row, "FRONT_HOST_NAME", enFrontHostInfo.getHostNameCn()+"("+enFrontHostInfo.getHostNameEn()+")");
				}
				if(enLocationInfo != null ){
					requestXml.setItemValue("DEVICE_INFO", row, "LOCATION_NAME", enLocationInfo.getLocationNameCn()+"("+enLocationInfo.getLocationNameEn()+")");
				}
