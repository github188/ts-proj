...
public class BoDeviceSubmit implements RootBo {
	...
	String userPrompt; // 登录用户名提示符
	String passwordPrompt; // 登录密码提示符
	...
	sql1 = new StringBuffer(" DEVICE_NAME_EN ='" + deviceNameEn + "'");

	sql2 = new StringBuffer(" DEVICE_ABB_NAME_EN ='" + deviceAbbNameEn + "'");

	sql3 = new StringBuffer(" DEVICE_NAME_CN ='" + deviceNameCn + "'");
	...
	enDeviceInfo.setUserPrompt(userPrompt);
	enDeviceInfo.setPasswordPrompt(passwordPrompt);
	// 判断是添加还是编辑：
	if (deviceId == null || deviceId.length() == 0) {
	    deviceId = SysIdCreator.GenNextId(transaction, null, IdCreatorDefine.ID_TYPE_DEVICE_ID,
