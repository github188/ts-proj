...
/**
 * 功能描述：为维护团队增加维护人员 1、根据维护团队编号从维护团队与人员对照表（MAINTAIN_TEAM_USER_MAP）中删除所有对应维护人员信息。
...
public class BoMaintainPersonSubmit implements RootBo {
	public void doBusiness(Transaction transaction, XMLWrap requestXml, XMLWrap sessionXml,
		...
		for (int i = 0; i < userIds.length; i++) {
			int iCount = dbMaintainTeamUserMap.countWhere("TEAM_ID ='" + teamId + "' and USER_ID ='"
					+ userIds[i] + "'");
			if (iCount < 1) {
				mapId = SysIdCreator.GenNextId(transaction, null, IdCreatorDefine.ID_TYPE_TEAM_USER_MAP_ID,
