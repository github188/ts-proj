...
public class BoFrontHostList implements RootBo {
	...
	if (sqlWhere != null && sqlWhere.length() != 0) {
	    Page.SetPageInfo(transaction, null, requestXml, dbFrontHostInfo, PubFunc.LEN_PAGE_COUNT,
		    "FRONT_HOST_INFO", sqlWhere.toString());
	    hosts = dbFrontHostInfo.findAllWhere(sqlWhere.toString());

	} else {
	    Page.SetPageInfo(transaction, null, requestXml, dbFrontHostInfo, PubFunc.LEN_PAGE_COUNT,
		    "FRONT_HOST_INFO", null);
	...
	for (int i = 0; i < hosts.size(); i++) {
		...
		requestXml.setItemValue("FRONT_HOST_INFO", row, "LOCATION_NAME", enLocationInfo
			.getLocationNameCn());// +"("+enLocationInfo.getLocationNameEn()+")");
	    }
