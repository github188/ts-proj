	...
	public void doBusiness(Transaction transaction, XMLWrap requestXml, XMLWrap sessionXml, XMLWrap applicationXml, Logger logger) throws ErrorException {
		 ...
		 **********************************************************************/
		 sql.append("select a.SEND_ID,b.DEVICE_NAME_EN,b.DEVICE_NAME_CN,e.TYPE_NAME_CN,b.DEVICE_IP,a.TASK_DEFINE_TIME," +
		 		" d.STATUS,d.INSPECT_END  " +
		 		"from COMMANDS_SEND_HIS a  " +
		 		"left join DEVICE_INFO b on a.DEVICE_ID = b.DEVICE_ID " +
		 		"left join DEVICE_INSPECT_LOG d on a.SEND_ID = d.SEND_ID " +
		 ...
		 sql.append(Transaction.formatString("I"));
		 //根据查询条件组装查询语句
		 ...
		 sqlWhere = new StringBuffer();
			...
			if (execEndBegin != null && execEndBegin.length() != 0) {
				if (sqlWhere == null || sqlWhere.length() == 0) {
					sqlWhere.append(" a.INSPECT_END  >='" + execEndBegin + "'");
				} else {
					sqlWhere.append(" AND a.INSPECT_END  >='" + execEndBegin + "'");
				}
			}
			if (execEndEnd != null && execEndEnd.length() != 0) {
				if (sqlWhere == null || sqlWhere.length() == 0) {
					sqlWhere.append(" a.INSPECT_END  >='" + execEndEnd + "'");
				} else {
					sqlWhere.append(" AND a.INSPECT_END  >='" + execEndEnd + "'");
				}
			}
			...
			for(int i=0; i<rs.size(); i++){
				...
				if(rsRow != null){
					...
					requestXml.setItemValue("DEVICE_INSPECT_LOG", row, "TYPE_NAME", rsRow.getString("TYPE_NAME"));
					requestXml.setItemValue("DEVICE_INSPECT_LOG", row, "DEVICE_IP", rsRow.getString("DEVICE_IP"));
					requestXml.setItemValue("DEVICE_INSPECT_LOG", row, "TASK_DEFINE_TIME", rsRow.getString("TASK_DEFINE_TIME"));
					requestXml.setItemValue("DEVICE_INSPECT_LOG", row, "STATUS", rsRow.getString("STATUS"));
					requestXml.setItemValue("DEVICE_INSPECT_LOG", row, "INSPECT_END", rsRow.getString("INSPECT_END"));
				}
			}
