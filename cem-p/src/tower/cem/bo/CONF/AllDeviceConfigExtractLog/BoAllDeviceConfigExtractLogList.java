...
public class BoAllDeviceConfigExtractLogList implements RootBo{
	 ...
	 public void doBusiness(Transaction transaction, XMLWrap requestXml, XMLWrap sessionXml,
			...
			// 查询条件
			String execEndBegin; // 提取结束时间
			String execEndEnd;   // 提取结束时间
			...
			/***********************************************************************
		         * 获取输入
		         **********************************************************************/
			execEndBegin = requestXml.getInputValue("EXTRACT_END_BEGIN");
			execEndEnd = requestXml.getInputValue("EXTRACT_END_END");
			/***********************************************************************
		         * 创建数据库连接、实例化DB、EN
		         **********************************************************************/
			...
			sql
				.append(" select a.SEND_ID,a.TASK_DEFINE_TIME, a.TASK_PLAN_TIME,a.STATUS,a.EXEC_BEGIN_TIME,a.EXEC_END_TIME,f.USER_NAME, a.DEVICE_TYPE_ID"
					...
					+ "  a.USER_ID  = f.USER_ID "
					+ " and a.COMMANDS_TYPE = 'E' " + " and a.DEVICE_ID is  null ");
