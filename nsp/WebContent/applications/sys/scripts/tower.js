...
function selDialog(url,idInput,nameInput,flagInput,popWidth,popHeight,scrollIt,winName){
	if(winName == null){
		winName = "_blank";
	}
	if(popWidth == null){
	...
	if(result){
		document.all(idInput).value = result[0];
		document.all(nameInput).value = result[1];
		document.all(flagInput).value = result[2];
		return true;
	} else {
		return false;
	}
}
function selDialog(url,idInput,nameInput,idParent,flagInput,popWidth,popHeight,scrollIt,winName){
	if(winName == null){
		winName = "_blank";
	}
	if(popWidth == null){
	...
	if(result){
		document.all(idInput).value = result[0];
		document.all(nameInput).value = result[1];
		document.all(idParent).value = result[2];
		document.all(flagInput).value = result[3];
		return true;
	} else {
		return false;
	}
...
function closeDialog(idValue,nameValue,idFlag){
	var result = new Array(idValue,nameValue,idFlag);
	window.returnValue = result;
	window.close();
}
function closeDialog(idValue,nameValue,idParent,idFlag){
	var result = new Array(idValue,nameValue,idParent,idFlag);
	window.returnValue = result;
	window.close();
}
