	...
	public void doBusiness(Transaction transaction, XMLWrap requestXml, XMLWrap sessionXml,
		...
		for (int i = 0; i < qr.size(); i++) {
			...
			if (enResourceOrgAmount != null) {
				...
				requestXml.setItemValue("RESOURCE_ORG_AMOUNT", row, "ONLINE_AMOUNT_CONF_AMOUNT", String
						.valueOf(online));
				requestXml.setItemValue("RESOURCE_ORG_AMOUNT", row, "CLASS_NAME", ritem
						.getString("CLASS_NAME"));
				requestXml
						.setItemValue("RESOURCE_ORG_AMOUNT", row, "TYPE_NAME", ritem.getString("TYPE_NAME"));
			}
