		...
		if (orgId == null || orgId.length() == 0) {
			...
			if (resourceOuts != null && resourceOuts.size() > 0) {
				for (int i = 0; i < resourceOuts.size(); i++) {
					...
					enResourcePrepareSheet = dbResourcePrepareSheet.findByKey(enResourcePrepareList
							.getSheetId());
					...
					requestXml.setItemValue("RESOURCE_PREPARE_LIST", row, "IN_ORG_NAME", enSysOrg
							.getOrgName());
					requestXml.setItemValue("RESOURCE_PREPARE_LIST", row, "IN_STATION_NAME", inStation);
					requestXml.setItemValue("RESOURCE_PREPARE_LIST", row, "OUT_ORG_NAME", enSysOrgOut
							.getOrgName());
