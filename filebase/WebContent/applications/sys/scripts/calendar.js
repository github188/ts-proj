...
Calendar.prototype.parseDate = function (str, fmt) {
	...
	var pos = fmt.indexOf("ymmdd");
	if(pos >= 0){
		var fmtLeft = fmt.substring(0,pos);
		var fmtCenter = fmt.substring(pos,pos+5);
		var fmtRight = fmt.substring(pos+5);
		var dateStr = new String(str);

		if(fmtLeft==""||fmtLeft==null){
			pos = -1;
		} else {
			pos = dateStr.indexOf(fmtLeft);
		}
		if(pos>=0){
			dateStr = dateStr.substring(fmtLeft.length);
		}
	
		
		if(fmtRight==""||fmtRight==null){
			pos = -1;
		} else {
			pos = dateStr.indexOf(fmtRight);
		}
		if(pos>=0){
