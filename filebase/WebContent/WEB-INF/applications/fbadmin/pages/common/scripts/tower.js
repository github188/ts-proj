/*检查日期输入顺序是否正确*/
function checkDate(inputFirstName,inputLastName){
    
    if(document.all(inputFirstName) == null){
        alert("变量未定义！");
        return false;
    }
    if(document.all(inputLastName) == null){
        alert("变量未定义！");
        return false;
    }
    var tmpFirst = document.all(inputFirstName).length;
	var tmpLast = document.all(inputLastName).length;
		if(( (typeof tmpFirst) == "undefined")&&( (typeof tmpLast) == "undefined") ) {
			var firstDate=document.all(inputFirstName).value;
			var lastDate=document.all(inputLastName).value;
			if(!(firstDate=='')&&!(lastDate=='')){
			    var tmpFirst=cTrim(firstDate);
			    var tmpLast=cTrim(lastDate);
			    if(parseInt(tmpLast)<parseInt(tmpFirst)){
			        alert('日期输入顺序不符合要求，请重新输入！');
			        return false;
			    }
			}
		    return true;
...
function cTrim(sInputString){
   var tmp='';
   var sTmpStr = sInputString.split("-");
   for (loop=0; loop < sTmpStr.length;loop++){
　　       tmp=tmp+sTmpStr[loop];
　　}
