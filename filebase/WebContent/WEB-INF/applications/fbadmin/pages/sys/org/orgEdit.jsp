...
<%
  ...
  //编辑信息
  selectFlag=xml.getInputValue("SELECT_FLAG");
...
<script type="text/javascript">
  ...
  <%if(selectFlag!=null&&selectFlag.length()>0){
          if(selectFlag.equals("up")){
    %>
           form1.SELECT_ORG_ID.value='<%=orgId %>';
     <%  }
         if(selectFlag.equals("down")){
     %>  
          form1.SELECT_ORG_ID.value='<%=parentId%>'; 
    <%   }
      }%>
    if(!phoneCheck(document.all.LINK_TELE.value)){
    	return false;
    }
  ...
  function doCancle(id) {
     <%if(selectFlag!=null&&selectFlag.length()>0){
          if(selectFlag.equals("up")){
    %>
            window.location="ctrl?FUNC_ID=OrgList&PARENT_ID=<%=orgId%>"; 
     <%  }
         if(selectFlag.equals("down")){
     %>  
         window.location="ctrl?FUNC_ID=OrgList&PARENT_ID=<%=parentId%>"; 
    <%   }
      }%>
...
<div id="noticePanel" class="panelSimple">
                      ...
                      <div class="panelContent">
                          <div class="panelContent2">
                			...
                			<input type="hidden" name="SELECT_ORG_ID" value=""> 
              				<table width="490" border="0" cellpadding="0" cellspacing="0" class="list">
               				 <tr>
                  <td width="95" align="right">机构名称：</td>
...
var sprytextfield3 = new Spry.Widget.ValidationTextField("linkTele", "phone_number", {format:"phone_custom",isRequired:false,maxChars:12});
var sprytextfield4 = new Spry.Widget.ValidationTextField("spryLinkEmail", "email", {isRequired:false, maxChars:50});
var sprytextarea1 = new Spry.Widget.ValidationTextarea("spryOrgDesc", {maxChars:200, isRequired:false});
//-->
</script>
</body>
</html>
