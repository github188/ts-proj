...
public class BoCatalogDestory implements RootBo {
	 ...
	 * <strong>输入：</strong><br>
	 * <br>
	 * 文件代号：CONTENT_ID <strong>业务逻辑：</strong><br>
	 * <br>
	 * 根据文件目录代号（CONTENT_ID）从目录表中删除目录及下级目录及目录下的所有文件 <strong>输出：</strong><br>
	 * <br>
	 * 无 <br>
	 */
	public void doBusiness(Transaction transaction, XMLWrap requestXml,
		 ...
		 * 执行业务逻辑、输出
		 **********************************************************************/
		if(requestXml.getInputRowCount("PARENT_ID")==0){
			requestXml.addInputRow("PARENT_ID");
		}
		enTCatalog = dbTCatalog.findByKey(catalogId);
		if(enTCatalog != null){
			requestXml.setInputValue("PARENT_ID", 1,enTCatalog.getParentId());
		}
		
			// 删除目录及下级所有目录和文件
			nextCatalog.push(catalogId);
			while (!nextCatalog.isEmpty()) {

				catalogs = dbTCatalog.findAllWhere(" parent_id = '"
						+ nextCatalog.peek() + "'");
				// 判断目录下是否存在子目录
	...
	/**
	 * 功能描述：根据目录contentId删除目录 filePath：系统路径
	 * 
	 * @param transaction
	 * @param contentId
	 * @param filePath
	 * @throws ErrorException
	 */
	public void deleCatalog(Transaction transaction, String contentId,
			String filePath) throws ErrorException {
		...
		if (contentId != null && contentId.length() > 0) {
			...
			// 获得目录的相对路径
			String partPath = PathByCatalog.pathByCatalogId(contentId,
					transaction);
	...
	/**
	 * 功能描述：根据目录contentId删除目录下的所有文件及文件版本信息 filePath：系统路径
	 * 
	 * @param transaction
	 * @param contentId
	 * @param filePath
	 * @throws ErrorException
	 */
	public void deleFile(Transaction transaction, String contentId,
			String filePath) throws ErrorException {
		// 文件信息
		Vector files;
		DbTFile dbTFile;
		...
		if (files != null && files.size() > 0) {
			...
			for (int i = 0; i < files.size(); i++) {
				...
				// 获取文件的版本
				fileVersions = dbTFileVersion.findAllWhere(" FILE_ID = '"
						+ enTFile.getFileId() + "'");

				if (fileVersions != null && fileVersions.size() > 0) {

					// 删除文件表的版本记录
					dbTFileVersion.deleteWhere(" FILE_ID = '"
							+ enTFile.getFileId() + "'");
